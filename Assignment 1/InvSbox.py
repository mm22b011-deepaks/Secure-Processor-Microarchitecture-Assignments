...
def calculate_final_xor(k3, k4):
    ...
    for i in range(256):
        ...
        output_matrix[i] = inv_mixed[3]  # Ensure it's within 0x00 to 0xFF
    
    	
    	
    # Apply the inverse S-Box transformation
    inverse_sbox_output = [inv_sbox[output_matrix[i]] for i in range(256)]
    
    

    # XOR all S-Box inverse outputs
    final_xor_result = 0x00
    for value in inverse_sbox_output:
        final_xor_result ^= value
...
for k3 in range(0x00, 0x100):
    for k4 in range(0x00, 0x100):
        ...
        keys[2] = k3
        keys[3] = k4
        # Calculate final XOR result with current k3 and k4
        result = calculate_final_xor(k3, k4)
        if result == 0x00:
            valid_keys.append((k3, k4))

# Display valid keys
print("Valid k3 and k4 pairs where final XOR result is 00:")
