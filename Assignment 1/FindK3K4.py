...
def calculate_final_xor(k3, k4):
    ...
    inverse_sbox_output = [[inv_sbox[output_matrix[i][j]] for j in range(4)] for i in range(256)]

    # XOR all bytes of each column
    final_xor_results = [0x00 for _ in range(4)]
    for j in range(4):
        for i in range(256):
            final_xor_results[j] ^= inverse_sbox_output[i][j]
...
# Display valid keys
print("Valid k3 and k4 pairs where final XOR result for all bytes is 00:")
